...
- (void)createMainBtnWithFrame:(CGRect)frame{
    ...
    if ([dataSour.tagString isEqualToString:@"6"]) {
        [_mainBtn setTitle:dataSour.selectArtificialStr forState:UIControlStateNormal];
    } else {
        [_mainBtn setTitle:@"点击选择" forState:UIControlStateNormal];
    }
...
- (void)setMenuTitles:(NSArray *)titlesArr rowHeight:(CGFloat)rowHeight{
    ...
    _listView.frame = CGRectMake(VIEW_X(self) , VIEW_Y_Bottom(self), VIEW_WIDTH(self), HT-200);
...
- (void)showDropDown{   // 显示下拉列表
    ...
    [UIView animateWithDuration:AnimateTime animations:^{
        ...
        _listView.frame  = CGRectMake(VIEW_X(_listView), VIEW_Y(_listView), VIEW_WIDTH(_listView), HT-200);//_rowHeight *_titleArr.count
