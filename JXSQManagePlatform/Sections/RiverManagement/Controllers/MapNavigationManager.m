...
- (instancetype)init
{
    ...
    if (self) {
        ...
        self.urlScheme = @"SiteConstruction://";
        self.appName = @"SiteConstruction";
    }
    return self;
}


- (void)showSheetAddView:(UIView *)aview
{
    ...
    _titArr = [NSMutableArray arrayWithObjects:appleMap,baiduMap,googleMap,tencentMap, nil];
...
- (void)actionSheet:(UIActionSheet *)actionSheet clickedButtonAtIndex:(NSInteger)buttonIndex
{
    ...
    if (_titArr.count == buttonIndex) {
        NSLog(@"+++++++++++++ %lu ============== %ld", (unsigned long)_titArr.count,(long)buttonIndex);
    } else {
...
- (void)startNavigation:(MapSelect)index
{
    ...
    else if(_style == Coordinates)
    {
        ...
        MKMapItem *toLocation = [[MKMapItem alloc] initWithPlacemark:[[MKPlacemark alloc] initWithCoordinate:_Coordinate2D addressDictionary:nil]];
