...
//    }];
    return [[PHImageManager defaultManager] requestImageDataForAsset:asset options:nil resultHandler:^(NSData * _Nullable imageData, NSString * _Nullable dataUTI, UIImageOrientation orientation, NSDictionary * _Nullable info) {
        if (imageData) {
            NSString *suffix = [[info objectForKey:@"PHImageFileURLKey"] pathExtension];
...
//            NSData *imgdata = UIImageJPEGRepresentation(image, 1);
            ...
            if ([suffix isEqualToString:@"heic"]||[suffix isEqualToString:@"heif"]||[suffix isEqualToString:@"HEIF"] || [suffix isEqualToString:@"HEIC"]) {
                ...
                if (completion) completion(jpgData,info);
            } else {
                if (completion) completion(imageData,info);
            }
        }
...
+ (void)sortImageForModel:(HXPhotoModel *)model total:(NSInteger)total images:(NSMutableArray *)images completion:(void(^)(NSArray *array))completion {
    ...
    if (images.count == total) {
        ...
        for (HXPhotoModel *md in images) {
            if (!md.previewPhoto) {
                UIImage *tempImage = md.thumbPhoto;
//                tempImage.tagData = md;
                [array addObject:md.thumbPhoto];
                
            }else {
                UIImage *tempImage = md.previewPhoto;
//                tempImage.tagData = md;
