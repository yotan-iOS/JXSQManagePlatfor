...
//        [sheet showInView:self];
        ...
        UIAlertController *alertController = [UIAlertController alertControllerWithTitle:@"获取图片" message:nil preferredStyle: UIAlertControllerStyleAlert];
...
- (void)setupNewFrame {
    ...
    CGFloat itemW = (width - self.Spacing * (self.LineNum - 1)) / self.LineNum;
    if (itemW > 0) {
        self.flowLayout.itemSize = CGSizeMake(itemW, itemW);
    }
    
    NSInteger dataCount = self.dataList.count;
    NSInteger numOfLinesNew = (dataCount / self.LineNum) + 1;
    
    if (dataCount % self.LineNum == 0) {
        numOfLinesNew -= 1;
    }
    self.flowLayout.minimumLineSpacing = self.Spacing;
